...
let handler = async (m, { text, usedPrefix, command }) => {
    if (!text) throw `*_Please provide a plugin URL_*`;
 ...
 const gistId = text.match(/(?:\/|gist\.github\.com\/)([a-fA-F0-9]+)/);
    ...
    try {
        ...
        for (const file of Object.values(gistData.files)) {
            ...
            m.reply(`𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙞𝙣𝙨𝙩𝙖𝙡𝙡𝙚𝙙 𝙩𝙝𝙚 𝙥𝙡𝙪𝙜𝙞𝙣 𝙩𝙤 𝙂𝘿𝙎-𝙈𝘿`);
        }
    } catch (error) {
...
handler.help = ['installp'].map((v) => v + ' <Gist URL>');
handler.tags = ['plugin'];
handler.command = /^installp$/i;
